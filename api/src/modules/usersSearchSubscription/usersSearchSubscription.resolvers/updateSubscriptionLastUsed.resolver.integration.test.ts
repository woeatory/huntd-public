...
describe(`Test resolver: updateSubscriptionLastUsed`, testFactory(() => {
  let user: User;
  let client: Sdk;
  ...
  it(`should throw 'unauthorized' error when called anonymously`, async () => {
    await expect(
      global.client().updateSubscriptionLastUsed({
        id: 1,
        userId: 1,
      }),
    )
  ...
  it(`should throw 'forbidden' error when called with wrong userId`, async () => {
    ...
    await expect(
      client.updateSubscriptionLastUsed({
  ...
  it(`should update 'lastUsed' field`, async () => {
    const existingSubscription = await factory.usersSearchSubscription.create({
      userId: user.id,
    });

    const initialLastUsed = existingSubscription.lastUsed;

    await client.updateSubscriptionLastUsed({
      id: existingSubscription.id,
      userId: user.id,
    });

    await existingSubscription.reload();

    expect(initialLastUsed)
      .not.toEqual(existingSubscription.lastUsed);
  });
}));
